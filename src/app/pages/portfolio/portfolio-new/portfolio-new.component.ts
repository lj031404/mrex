...
export class PortfolioNewComponent implements OnInit, OnDestroy {
	...
	initForm(config: PortfolioConfig) {
		this.form = this.fb.group({
			...
			propertyType: [config.propertyType, Validators.required],
		...
		const initialValue = _.cloneDeep(this.form.value)
		...
		this.sub.add(
			this.form.valueChanges.pipe(
				map(value => _.isEqual(initialValue, value))
			).subscribe(res => {
				this.isChanged = !res
			})
		)
		...
		this.route.queryParams.subscribe(async (param) => {
			...
			try {
				if (propertyId) {
					...
					this.listPropertyEvents = _.sortBy(propertyEvents.filter(event => event.type ===  EventType.Sold), 'date').reverse()
